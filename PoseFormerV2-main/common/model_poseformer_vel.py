...
class PoseTransformerV2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2,
        ...
        # === motion channels: (x,y) + (dx,dy) + (ddx,ddy) ===
        # keep external API the same (datapipes still pass 2 channels);
        # we build extra channels internally at runtime.
        self.in_chans_raw = in_chans                    # 2
        self.in_chans_aug = in_chans * 2               # 4: (pos, vel)

        ### spatial patch embedding
        # self.Joint_embedding = nn.Linear(in_chans, embed_dim_ratio)
        # self.Freq_embedding = nn.Linear(in_chans*num_joints, embed_dim)
        self.Joint_embedding = nn.Linear(self.in_chans_aug, embed_dim_ratio)
        self.Freq_embedding = nn.Linear(self.in_chans_aug * num_joints, embed_dim)
    ...
    @staticmethod
    def _build_vel_channels(x, smooth: bool = False):
        """
        x: (B, F, J, 2) raw 2D joints
        return: (B, F, J, 4) = (x, y, dx, dy)
        Pads vel[t=0] = 0 so shape stays (B,F,...)
        If smooth=True, applies a simple 3-frame moving average before differencing.
        """
        if smooth and x.size(1) >= 3:
            # simple 3-frame box filter on positions to reduce jitter
            x_sm = x.clone()
            x_sm[:, 1:-1] = (x[:, 0:-2] + x[:, 1:-1] + x[:, 2:]) / 3.0
            x_for_diff = x_sm
        else:
            x_for_diff = x

        vel = torch.zeros_like(x)
        vel[:, 1:] = x_for_diff[:, 1:] - x_for_diff[:, :-1]   # dx, dy
        x_aug = torch.cat([x, vel], dim=-1)                   # (B, F, J, 4)
        return x_aug
    ...
    def forward(self, x):
        ...
        b, f, p, _ = x.shape
        # build motion channels once; use for both spatial & freq branches
        x_aug = self._build_vel_channels(x, smooth=True)         # (B,F,J,4)

        Spatial_feature = self.Spatial_forward_features(x_aug)
        x = self.forward_features(x_aug, Spatial_feature)
