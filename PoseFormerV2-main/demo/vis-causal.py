...
def get_pose3D(video_path, output_dir):
    ...
    print(f"Video frame count: {video_length}")
    ...
    for i in tqdm(range(video_length)):
        ...
        # Modify the frame window for online inference (no future frames known)
        start = max(0, i - args.pad)  # Past frames (left side)
        end = i  # Current frame only (it should be the center)

        # Get the past frames (before the current one)
        input_2D_no = keypoints[0][start:end+1]

        # Pad the future frames (after the current one) with the current frame
        # We need to pad the right side with the current frame (i.e., future frames become the current frame)
        if len(keypoints[0]) > i + args.pad:
            future_frames = np.repeat(keypoints[0][i:i+1], args.pad, axis=0)  # Repeat the current frame for future frames
        else:
            future_frames = np.repeat(keypoints[0][i:i+1], args.pad - (len(keypoints[0]) - 1 - i), axis=0)
            
        joints_left =  [4, 5, 6, 11, 12, 13]
        joints_right = [1, 2, 3, 14, 15, 16]

        # Concatenate the past frames with the padded future frames
        input_2D_no = np.concatenate((input_2D_no, future_frames), axis=0)

        # Normalize the 2D keypoints
...
if __name__ == "__main__":
    ...
    # SKIP 2D POSE GENERATION
    # Use your manually prepared 2D keypoints instead

    # Copy keypoints file to the expected path (optional, if not already done)
    keypoints_src = './demo/output/test/input_2D/keypoints_17.npz'
    keypoints_dst = output_dir + 'input_2D/keypoints.npz'
    os.makedirs(os.path.dirname(keypoints_dst), exist_ok=True)
    if not os.path.exists(keypoints_dst):
        import shutil
