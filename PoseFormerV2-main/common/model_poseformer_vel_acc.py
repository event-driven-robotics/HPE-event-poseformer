...
class PoseTransformerV2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2,
        ...
        self.in_chans_aug = in_chans * 3               # 6: (pos, vel, acc)
    ...
    @staticmethod
    def _build_motion_channels(x):
        """
        x: (B, F, J, 2) raw 2D joints
        returns: (B, F, J, 6) = (x,y, dx,dy, ddx,ddy)
        zero-pads the first 1/2 steps for vel/acc respectively
        """
        vel = torch.zeros_like(x)
        vel[:, 1:] = x[:, 1:] - x[:, :-1]              # (B,F,J,2), vel[t] = x[t]-x[t-1]
        acc = torch.zeros_like(x)
        acc[:, 2:] = vel[:, 2:] - vel[:, 1:-1]         # (B,F,J,2), acc[t] = vel[t]-vel[t-1]
        x_aug = torch.cat([x, vel, acc], dim=-1)       # (B,F,J,6)
        return x_aug
    ...
    def forward(self, x):
        ...
        x_aug = self._build_motion_channels(x)         # (B,F,J,6)
