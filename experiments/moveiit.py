...
sys.path.append(os.path.join(os.getcwd(), 'submodules/bimvee'))
...
def _find_event_file(root: str):
    """
    Find the *actual* event file under a folder, so we pass a single file to importAe.
    Priority:
      1) data.log
      2) binaryevents.log
      3) *.h5
    ...
    for name in ["data.log", "binaryevents.log"]:
        cand = glob.glob(os.path.join(root, "**", name), recursive=True)
        if cand:
            return cand[0]

    cand = glob.glob(os.path.join(root, "**", "*.h5"), recursive=True)
    if cand:
        return cand[0]

    cand = glob.glob(os.path.join(root, "**", "*.log"), recursive=True)
    if cand:
        return cand[0]

    raise FileNotFoundError(f"No event file found under: {root}")


def import_file(data_dvs_path: str, prefer_key: str = None):
    """
    Robust import using bimvee.importAe, but constrained to ONE event file and non-recursive.
    Returns (dvs_stream, stream_key)
    """
    print(f"[import_file] input={data_dvs_path}")
    event_file = _find_event_file(data_dvs_path)
    print(f"[import_file] using event file: {event_file}")

    importers = importAe(filePathOrName=event_file)

    if "data" not in importers:
        raise KeyError("Missing 'data' in importAe output. Check event_file and bimvee version.")

    data_keys = list(importers["data"].keys())
    if not data_keys:
        raise KeyError("No streams found under importers['data'].")

    if prefer_key is not None and prefer_key in data_keys:
        chosen_key = prefer_key
    elif len(data_keys) == 1:
        chosen_key = data_keys[0]
    else:
        dvs_like = [k for k in data_keys if ("dvs" in k.lower() or "event" in k.lower())]
        chosen_key = dvs_like[0] if dvs_like else data_keys[0]

    dvs_stream = importers["data"][chosen_key]["dvs"]

    print(f"[import_file] loaded stream: '{chosen_key}'")
    print(f"[import_file] ts: {dvs_stream.get_first_ts():.6f}s -> {dvs_stream.get_last_ts():.6f}s")

    return dvs_stream, chosen_key

def save_event_video_and_csv(data_dvs_file, output_path, args):
    """ Process event data and generate videos, CSVs, and 3D pose estimations. """
    init(cfg)
    ...
    dvs_stream, stream_key = import_file(data_dvs_file, prefer_key=getattr(args, "stream_key", None))

    # Generate synthetic timestamps at desired FPS
    start_ts = float(dvs_stream.get_first_ts()) / args.ts_scaler
    end_ts = float(dvs_stream.get_last_ts()) / args.ts_scaler
    data_ts = {"ts": list(np.arange(start_ts, end_ts, 1.0 / args.fps, dtype=np.float64))}
    print(f"Generated {len(data_ts['ts'])} frames from {start_ts:.3f}s to {end_ts:.3f}s at {args.fps} fps")

    # Get the chosen event representation
    rep = get_representation(args.rep, args)

    # Write CSV for keypoints if enabled
    if args.write_csv:
    ...
    # Processing frames and saving outputs
    dt = 1.0 / args.fps
    window_size = dt
    half = window_size / 2.0

    for fi, t in enumerate(data_ts["ts"]):
        rep.reset_frame()

        if fi % 100 == 0:
           print("frame: ", fi, "/", len(data_ts["ts"]))

        # Handle skipping frames based on user input
        if args.stop is not None and fi > args.stop:
            break
        ...
        e_data = dvs_stream.get_data_at_time(t, window_size)
        ex = np.asarray(e_data["x"])
        ey = np.asarray(e_data["y"])
        ets = np.asarray(e_data["ts"])

        # Skip empty frames
        if ex.size > 0:
            for x, y in zip(ex, ey):
                rep.update(vx=int(x), vy=int(y))
        ...
        if args.write_csv:
            pre = run_task.predict_online(frame, ts=t)
...
def load_model_weights(model, ckpt_path, device, gpu):
    ...
    try:
        ...
        raise e
def pred17_to_pred13(post_out_17):
    """
    post_out_17: (17,3) PoseFormer output in your pipeline space.
    returns: (13,3) in YOUR MoveNet 13 joint order:
    [HEAD, RSHOULDER, LSHOULDER, RELBOW, LELBOW, LHIP, RHIP, RWRIST, LWRIST, RKNEE, LKNEE, RANKLE, LANKLE]
    """
    # PoseFormer joint indices in your code:
    # 0 pelvis, 1 RHip,2 RKnee,3 RAnkle,4 LHip,5 LKnee,6 LAnkle, ...
    # 9 Nose(from head), 11 LShoulder,12 LElbow,13 LWrist, 14 RShoulder,15 RElbow,16 RWrist
    idx = [9, 14, 11, 15, 12, 4, 1, 16, 13, 2, 5, 3, 6]
    return post_out_17[idx, :]

# def save_predictions_3d_npz(pred_list, output_dir, filename):
#     arr = np.stack(pred_list, axis=0)  # (T,J,3)
#     out = os.path.join(output_dir, filename)
#     np.savez_compressed(out, predictions=arr)
...
def get_pose3D(video_path, output_dir, save_images=True, save_demo=True, gpu=False, causal=False):
    ...
    pred13 = [pred17_to_pred13(p) for p in all_predictions]
    save_predictions_3d(pred13, video_path, "predictions_3d_13.npz")
